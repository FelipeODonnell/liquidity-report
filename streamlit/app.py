"""
Main application file for the Izun Crypto Liquidity Report.

This serves as the entry point for the Streamlit application and displays
the liquidity report dashboard as the home page.
"""
...
try:
    ...
    from utils.config import APP_TITLE, APP_ICON, ASSET_COLORS, DEFAULT_ASSET, EXCHANGE_COLORS, DATA_BASE_PATH

    logger.info(f"Successfully imported all modules")
    logger.info(f"Data path is set to: {DATA_BASE_PATH}")
...
def main():
    ...
    try:
        ...
        if not os.path.exists(DATA_BASE_PATH):
            st.error(f"Data directory not found. Please ensure data is available at: {DATA_BASE_PATH}")
            st.info("The application is looking for a directory structure with date folders (YYYYMMDD format) containing parquet files.")
            return
